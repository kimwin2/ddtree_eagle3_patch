...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Override the target model attention backend. Use eager when validating DDTree exactness against an eager verifier.",
    )
    ...
    if draft_algorithm == "eagle3" and target_attn_implementation == "flash_attention_2":
        ...
        logger.warning(
            "Loading the target with torch.sdpa for prefill/linear paths; DDTree switches branch-tree verification to eager internally for exact masked attention."
        )
    elif target_attn_implementation == "eager":
        logger.warning("Loading the target with eager attention; this is the apples-to-apples exactness mode for DDTree's eager tree verifier.")
    ...
    print(
        f"[INIT] Rank {dist.rank()}/{dist.size()} | "
        f"dataset={args.dataset} samples={args.max_samples} block_size={block_size} "
        f"mask_token_id={getattr(draft_model, 'mask_token_id', None)} "
        f"methods={methods_to_run} validate_exact_match={args.validate_exact_match}",
        flush=True,
    )
